...
/** DIFFERENT FUNCTIONS **/
vector<unsigned long int> findingPositionUsingMinimizers(string windowSeed, int windowLength, int q, int k, bool isForwardStrand) {
...
vector<unsigned long int> findingPositionUsingOpenAddr(string windowSeed, int windowLength, int q, int k, bool isForwardStrand) {
    vector<unsigned long int> foundLocations;
    vector<unsigned long int> location;
    unsigned long int rank;
...
vector<unsigned long int> findingPositionUsingDirAddr(string windowSeed, int windowLength, int q, int k, bool isForwardStrand) {
    vector<unsigned long int> foundLocations;
    vector<unsigned long int> location;
    unsigned long int rank;
    ...
    for (int i = 0; i < seeds.size(); i++) {
        ...
        for (int k = 0; k < j; k++) {
            ...
            if (windowLength >= seed.size()) {
                ...
                if (mode.compare("min") == 0) {
                    forward = findingPositionUsingMinimizers(seed, windowLength, q, k, isForward);
                } else if (mode.compare("dir") == 0) {
                    forward = findingPositionUsingDirAddr(seed, windowLength, q, k, isForward);
                } else if (mode.compare("open") == 0) {
                    forward = findingPositionUsingOpenAddr(seed, windowLength, q, k, isForward);
                }
        ...
        if (!isAccepted) {
            ...
            for (int k = 0; k < j; k++) {
                ...
                if (windowLength >= seed.size()) {
                    ...
                    if (mode.compare("min") == 0) {
                        reverse = findingPositionUsingMinimizers(seed, windowLength, q, k, isForward);
                    } else if (mode.compare("dir") == 0) {
                        reverse = findingPositionUsingDirAddr(seed, windowLength, q, k, isForward);
                    } else if (mode.compare("open") == 0) {
                        reverse = findingPositionUsingOpenAddr(seed, windowLength, q, k, isForward);
                    }
